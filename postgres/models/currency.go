...
type Currency struct {
	...
	MinDeposit       int64  `boil:"min_deposit" json:"min_deposit" toml:"min_deposit" yaml:"min_deposit"`
	InternalDecimals int    `boil:"internal_decimals" json:"internal_decimals" toml:"internal_decimals" yaml:"internal_decimals"`
...
var CurrencyTableColumns = struct {
	...
	InternalDecimals: "currency.internal_decimals",
}
...
var CurrencyWhere = struct {
	...
	MinDeposit       whereHelperint64
	InternalDecimals whereHelperint
}{
	Symbol:           whereHelperstring{field: "\"currency\".\"symbol\""},
	Name:             whereHelperstring{field: "\"currency\".\"name\""},
	Logo:             whereHelperstring{field: "\"currency\".\"logo\""},
	MinDeposit:       whereHelperint64{field: "\"currency\".\"min_deposit\""},
	InternalDecimals: whereHelperint{field: "\"currency\".\"internal_decimals\""},
}

// CurrencyRels is where relationship names are stored.
var CurrencyRels = struct {
	SymbolNetworkCurrencies string
}{
...
type currencyR struct {
	SymbolNetworkCurrencies NetworkCurrencySlice `boil:"SymbolNetworkCurrencies" json:"SymbolNetworkCurrencies" toml:"SymbolNetworkCurrencies" yaml:"SymbolNetworkCurrencies"`
}
...
var (
	currencyAllColumns            = []string{"symbol", "name", "logo", "min_deposit", "internal_decimals"}
	currencyColumnsWithoutDefault = []string{"symbol", "name", "logo"}
	currencyColumnsWithDefault    = []string{"min_deposit", "internal_decimals"}
	currencyPrimaryKeyColumns     = []string{"symbol"}
)
...
// SymbolNetworkCurrencies retrieves all the network_currency's NetworkCurrencies with an executor via symbol column.
func (o *Currency) SymbolNetworkCurrencies(mods ...qm.QueryMod) networkCurrencyQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"network_currency\".\"symbol\"=?", o.Symbol),
	)

	query := NetworkCurrencies(queryMods...)
	queries.SetFrom(query.Query, "\"network_currency\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"network_currency\".*"})
	}

	return query
}

// LoadSymbolNetworkCurrencies allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (currencyL) LoadSymbolNetworkCurrencies(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCurrency interface{}, mods queries.Applicator) error {
	var slice []*Currency
	var object *Currency
	...
	query := NewQuery(
		qm.From(`network_currency`),
		qm.WhereIn(`network_currency.symbol in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load network_currency")
	}

	var resultSlice []*NetworkCurrency
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice network_currency")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on network_currency")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for network_currency")
	}

	if singular {
		object.R.SymbolNetworkCurrencies = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &networkCurrencyR{}
			}
			foreign.R.SymbolCurrency = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.Symbol == foreign.Symbol {
				local.R.SymbolNetworkCurrencies = append(local.R.SymbolNetworkCurrencies, foreign)
				if foreign.R == nil {
					foreign.R = &networkCurrencyR{}
				}
				foreign.R.SymbolCurrency = local
				break
			}
		}
	}
...
// AddSymbolNetworkCurrencies adds the given related objects to the existing relationships
// of the currency, optionally inserting them as new records.
// Appends related to o.R.SymbolNetworkCurrencies.
// Sets related.R.SymbolCurrency appropriately.
func (o *Currency) AddSymbolNetworkCurrencies(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*NetworkCurrency) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.Symbol, rel.ID}
	...
	if o.R == nil {
		o.R = &currencyR{
			SymbolNetworkCurrencies: related,
		}
	} else {
		o.R.SymbolNetworkCurrencies = append(o.R.SymbolNetworkCurrencies, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &networkCurrencyR{
				SymbolCurrency: o,
			}
		} else {
			rel.R.SymbolCurrency = o
		}
	}
	return nil
}
...
func FindCurrency(ctx context.Context, exec boil.ContextExecutor, symbol string, selectCols ...string) (*Currency, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"currency\" where \"symbol\"=$1", sel,
	)
...
func (o *Currency) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"currency\" WHERE \"symbol\"=$1"
...
func (o *Currency) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindCurrency(ctx, exec, o.Symbol)
	if err != nil {
		return err
	}
...
func CurrencyExists(ctx context.Context, exec boil.ContextExecutor, symbol string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"currency\" where \"symbol\"=$1 limit 1)"
