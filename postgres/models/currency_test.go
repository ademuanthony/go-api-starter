...
func testCurrenciesExists(t *testing.T) {
	...
	e, err := CurrencyExists(ctx, tx, o.Symbol)
	if err != nil {
...
func testCurrenciesFind(t *testing.T) {
	...
	currencyFound, err := FindCurrency(ctx, tx, o.Symbol)
	if err != nil {
		t.Error(err)
	}
...
func testCurrencyToManySymbolNetworkCurrencies(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, networkCurrencyDBTypes, false, networkCurrencyColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, networkCurrencyDBTypes, false, networkCurrencyColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.SymbolNetworkCurrencies().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadSymbolNetworkCurrencies(ctx, tx, false, (*[]*Currency)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SymbolNetworkCurrencies); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.SymbolNetworkCurrencies = nil
	if err = a.L.LoadSymbolNetworkCurrencies(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SymbolNetworkCurrencies); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testCurrencyToManyAddOpSymbolNetworkCurrencies(t *testing.T) {
	var err error
	...
	foreigners := []*NetworkCurrency{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, networkCurrencyDBTypes, false, strmangle.SetComplement(networkCurrencyPrimaryKeyColumns, networkCurrencyColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*NetworkCurrency{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddSymbolNetworkCurrencies(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.Symbol != first.Symbol {
			t.Error("foreign key was wrong value", a.Symbol, first.Symbol)
		}
		if a.Symbol != second.Symbol {
			t.Error("foreign key was wrong value", a.Symbol, second.Symbol)
		}

		if first.R.SymbolCurrency != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.SymbolCurrency != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.SymbolNetworkCurrencies[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.SymbolNetworkCurrencies[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.SymbolNetworkCurrencies().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	currencyDBTypes = map[string]string{`Symbol`: `character varying`, `Name`: `character varying`, `Logo`: `character varying`, `MinDeposit`: `bigint`, `InternalDecimals`: `integer`}
	_               = bytes.MinRead
)
